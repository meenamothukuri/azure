...
func Test(t *testing.T) {
	...
	actualSqlId := terraform.Output(t, terraformOptions, "sql_id")
	actualSqldbId := terraform.Output(t, terraformOptions, "sqldb_id")

	// Assert that the actual values match the expected values
	// assert.Equal(t, expectedAgwId, actualAgwId)
	assert.NotNil(t, actualSqlId)
	assert.NotNil(t, actualSqldbId)
}